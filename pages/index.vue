      ...
      <v-row>
        <v-col v-for="category in categories" :key="category" cols="12">
          <v-card>
            ...
            <v-list two-line v-for="(course,n) in courses.filter(x=>x.category==category)" :key="course.code">
              <template >
                <div>
                  <v-list-item  >
                ...
                <v-divider
                  v-if="n !== courses.filter(x=>x.category==category).length-1"
                  :key="`divider-${n}`"
                  inset
                ></v-divider>
              </template>
